import { Message, Player, RoundTime } from '@paint-and-guess/lib/models';
...
SocketIO.listenOnSocket(EVENTS.FROM_SERVER.LEADERBOARD).subscribe((leaderboard: Pick<Player, 'name' | 'score'>[]) => {
    UI.renderLeaderboard(leaderboard);
});

SocketIO.listenOnSocket(EVENTS.FROM_SERVER.MESSAGE_HISTORY).subscribe(messages => {
    messages.forEach(message => UI.appendMessageToChat(message));
});
...
game.start$.subscribe((word: string) => {
    ...
    currentWordHeader.innerHTML = word
        .split('')
        .map(letter => (letter === '_' ? ' _ ' : letter))
        .join('');
});
...
game.gameState$.subscribe(gameState => {
    if (gameState.running) {
        ...
        currentWordHeader.innerHTML = gameState.revealedWord
            .split('')
            .map(letter => (letter === '_' ? ' _ ' : letter))
            .join('');
    }
...
game.allPlayer$.subscribe(() => {
    UI.renderOnlinePlayers(game.players);
    printPlayers();
});

SocketIO.emitOnSocket(PlayerActions.name$).subscribe(({ socket, socketData: name }) => {
    socket.emit(EVENTS.FROM_CLIENT.PLAYER_JOIN, name);
});

game.playerJoin$.subscribe((player: Player) => {
