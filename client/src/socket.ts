...
import { ClientToServerEvents, ServerToClientEvents } from '@paint-and-guess/lib/socket';

const socketIOURL = 'ws://localhost';
const socketIOPort = 1338;

export namespace SocketIO {
    const socket$: Observable<Socket<ServerToClientEvents, ClientToServerEvents>> = of(
        io(`${socketIOURL}:${socketIOPort}`)
    );
    ...
    export function emitOnSocket<T>(observable: Observable<T>): Observable<{
        ...
        return connection$.pipe(switchMap(socket => observable.pipe(map(socketData => ({ socket, socketData })))));
    }
}
