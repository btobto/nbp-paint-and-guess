...
import { Player } from '@paint-and-guess/lib/models';
...
export namespace PlayerActions {
    ...
    if (name) {
        ...
        name$ = of(prompt('Enter your name')).pipe(
            first(),
            filter(name => !!name),
    ...
    const thisPlayer$: Observable<Player> = combineLatest([SocketIO.connection$, name$]).pipe(
        map(([socket, name]) => ({ id: socket?.id, name, score: 0 }))
    );

    export const playerSentMessage$: Observable<Player> = combineLatest([thisPlayer$, send$]).pipe(
        map(data => data[0]),
        share()
    );
